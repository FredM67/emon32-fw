...
static void input(const uint32_t grp, const uint32_t pin, const bool high);
...
void portPinCfg(const uint32_t grp, const uint32_t pin, const uint32_t cfg,
                const PINCFG_t cs) {
  if (PIN_CFG_SET == cs) {
...
void portPinDir(const uint32_t grp, const uint32_t pin, const PINDIR_t mode) {
  if (PIN_DIR_IN == mode) {
...
void portPinDrv(const uint32_t grp, const uint32_t pin, const PINDRV_t drv) {
  switch (drv) {
  case PIN_DRV_CLR:
...
void portPinMux(const uint32_t grp, const uint32_t pin, const uint32_t mux) {
...
void portPinMuxClear(const uint32_t grp, const uint32_t pin) {
  PORT->Group[grp].PINCFG[pin].reg &= ~PORT_PINCFG_PMUXEN;
}

bool portPinValue(const uint32_t grp, const uint32_t pin) {
