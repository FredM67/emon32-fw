...
echo "OPTION A: Using UF2 Bootloader Upload (Easier)"
echo "1. Connect Arduino Zero to your computer via USB"
echo "2. Double-press the reset button to enter bootloader mode"
echo "3. Copy bin/emon32-performance-standard.uf2 to the EMONBOOT drive"
echo "4. Wait for device to restart - firmware runs automatically"
echo "5. Use serial monitor (115200 baud) to view results"
echo "6. Repeat steps 2-5 with bin/emon32-qfplib-performance.uf2"
echo ""
echo "OPTION B: Using probe-rs RTT (Advanced)"
echo "1. Connect Arduino Zero via USB (no bootloader mode needed)"
echo "2. For standard math: probe-rs run --chip ATSAMD21J17A target/thumbv6m-none-eabi/release/emon32-performance"
echo "3. For qfplib math: probe-rs run --chip ATSAMD21J17A target/thumbv6m-none-eabi/release/emon32-qfplib-performance"
echo "4. Compare the RTT output between both versions"
echo ""
echo -e "${YELLOW}📝 WSL Users:${NC} See WSL_SETUP_GUIDE.md for USB device sharing setup"
echo "   • Windows PowerShell: usbipd wsl attach --busid <BUSID>"
echo "   • File transfer: Use Windows Explorer → \\\\wsl\$\\Ubuntu\\..."
echo ""
